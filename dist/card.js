...
class $13bc0f3f43abb439$export$ac5c697e9c49bd62 extends HTMLElement {
    ...
    _computeSeverity(stateValue, sections) {
        ...
        let sortable = [];
        for(let severity in sections)sortable.push([
            severity,
            sections[severity]
        ]);
        sortable.sort((a, b)=>{
        ...
        if (numberValue >= sortable[0][1] && numberValue < sortable[1][1]) return severityMap[sortable[0][0]];
        if (numberValue >= sortable[1][1] && numberValue < sortable[2][1]) return severityMap[sortable[1][0]];
        if (numberValue >= sortable[2][1]) return severityMap[sortable[2][0]];
    ...
    set hass(hass) {
        ...
        if (config.measurement == null) {
            if (hass.states[config.entity] === undefined) measurement = 0;
            else measurement = hass.states[config.entity].attributes.unit_of_measurement;
        } else measurement = config.measurement;
